...
import com.axisrooms.db.query.AbstractInsertQuery.NextIdQuery;
import com.axisrooms.db.query.SqlQuery;

public class ArcQueryManager {

    private Connection          m_conn                = null;
    private boolean             m_isAutoCommitEnabled = true;
    private static final Logger s_logger              = Logger.getLogger(QueryManager.class);

    private ArcQueryManager() {
    }

    public static ArcQueryManager getInstance() {
        s_logger.debug("Start: Creating QueryManager.");
        ArcQueryManager queryManager = new ArcQueryManager();
        s_logger.debug("End: Creating QueryManager.");
        return queryManager;
    }

    public long executeWithIdSequence(SqlQuery query) throws Exception {
        boolean success = false;
        NextIdQuery nextId = null;
        if (query instanceof AbstractInsertQuery) {
            AbstractInsertQuery tmp = (AbstractInsertQuery) query;
            if (tmp.getSequenceName() != null) {
                nextId = tmp.new NextIdQuery();
                tmp.setNextIdQuery(nextId);
                success = execute(tmp.getNextIdQuery());
            }
            success = success && execute(tmp);
        }
        if (success) {
